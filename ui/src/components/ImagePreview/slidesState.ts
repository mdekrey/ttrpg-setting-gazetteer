import { createSubscribableList, createSubscribableValue } from "@/util/subscribables";
...
export function useSlidesState() {
	const slides = useSyncExternalStore((cb) => lightboxSlides.subscribe(cb), () => lightboxSlides.get(), () => lightboxSlides.get());
	const isLightboxOpen = useSyncExternalStore((cb) => lightboxOpen.subscribe(cb), () => lightboxOpen.get(), (): false => false);
